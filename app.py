...
if uploaded_file is not None:
    ...
    # Returns the specific missing column names so the error message is actionable.
    missing = [c for c in REQUIRED_COLUMNS if c not in df.columns]
    if missing:
        st.error(f"Missing required columns: {missing}. Please check your file and rename columns if needed.")
        st.stop()

    # STUDENT NOTE: Parse order_date as datetime. Without this, date arithmetic fails.
    ...
    # --- INTERACTIVE FILTER ---
    # STUDENT NOTE: State filter lets users narrow analysis to a specific region.
    # The filter is applied before metric computation — changing it reruns the
    # entire CLV formula on the filtered subset, changing all output values.
    ...
    # has multiple payment rows (e.g. installments).
    clv_df = (
    ...
    # STUDENT NOTE: Compute average order value — spending intensity per visit.
    ...
    clv_df['tenure_days'] = (clv_df['last_order_date'] - clv_df['first_order_date']).dt.days
    ...
    # regardless of the revenue scale or currency of the business using the tool.
    ...
    # STUDENT NOTE: Project 12-month CLV using daily purchase rate × average order value.
    # The +1 on tenure_days prevents division by zero for single-order customers.
    ...
    # STUDENT NOTE: Cap projections at 3× historical CLV to prevent unrealistic
    # outlier projections from inflating the total projected CLV figure.
    ...
    tier_summary = (
        ...
        .agg(customer_count=('customer_unique_id', 'count'),
             total_revenue=('total_revenue', 'sum'),
             avg_clv=('total_revenue', 'mean'))
        .reset_index()
    )

    # STUDENT NOTE: Chart 1 — Tier distribution bar chart with revenue per tier.
    fig1 = px.bar(
        tier_summary,
        x='clv_tier', y='total_revenue', text='customer_count',
    ...
    # STUDENT NOTE: Chart 2 — Tier profile: avg order value, frequency, tenure.
    ...
    for i, col in enumerate(['avg_order_value', 'order_count', 'tenure_days']):
        fig2.add_trace(
            go.Bar(x=tier_profile['clv_tier'],
                   y=tier_profile[col].round(1),
                   marker_color=[TIER_COLORS[t] for t in tier_profile['clv_tier']],
                   showlegend=False),
            row=1, col=i+1
        )
    fig2.update_layout(title_text='CLV Tier Profile Comparison')
    st.plotly_chart(fig2, use_container_width=True)

    # STUDENT NOTE: Chart 3 — CLV distribution histogram showing spread within tiers.
    cap = clv_df['total_revenue'].quantile(0.99)
    fig3 = px.histogram(
        clv_df[clv_df['total_revenue'] <= cap],
        x='total_revenue', color='clv_tier', nbins=80,
        title='CLV Distribution by Tier',
        labels={'total_revenue': 'Total Revenue', 'clv_tier': 'Tier'},
        color_discrete_map=TIER_COLORS, barmode='overlay', opacity=0.7
    )
    st.plotly_chart(fig3, use_container_width=True)

    # STUDENT NOTE: Full customer tier table — downloadable by the client.
    st.subheader("Customer Tier Table")
    display_df = clv_df[['customer_unique_id', 'total_revenue', 'order_count',
    ...
    **What to watch:** The 12-month projected CLV uses a simple linear model based on historical
    purchase rate. Customers with only one order will have low projections. Compare projected vs.
    historical CLV to identify customers who appear undervalued by the model.
    """)
```
